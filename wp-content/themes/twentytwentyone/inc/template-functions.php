...
function script_enqueuer() {
	...
	wp_register_script( 
		"vehicle_filter_script", 
		get_template_directory_uri() .'./assets/js/vehicle_filter.js', 
		array('jquery') 
	);
   
	// localize the script to your domain name, so that you can reference the url to admin-ajax.php file easily
	wp_localize_script( 'vehicle_filter_script', 'myAjax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' )));        
   
	// enqueue jQuery library and the script you registered above
	wp_enqueue_script( 'jquery' );
	wp_enqueue_script( 'vehicle_filter_script' );
}

add_action("wp_ajax_filter_vehicles", "filter_vehicles_handler");
add_action("wp_ajax_nopriv_filter_vehicles", "filter_vehicles_handler");

function filter_vehicles_handler()
{
	ob_start();
			
		$metaData = $_POST['data'];
		$vehicle_type = 	isset($metaData['vehicle_type']) ? $metaData['vehicle_type'] : '';
		$vehicle_title = 	isset($metaData['vehicle_title']) ? $metaData['vehicle_title'] : '';
		$vehicle_number = 	isset($metaData['vehicle_number']) ? $metaData['vehicle_number'] : '';
		$driver_name = 	isset($metaData['driver_name']) ? $metaData['driver_name'] : '';
		$vehicle_number_metaquery = [];
		$driver_name_metaquery = [];
		...
		if( $vehicle_number ) {
			$vehicle_number_metaquery = array(
				'meta_key' => 'eqs_vehicle_number',
				'value' => $vehicle_number
			);
		}
		
		if( $driver_name ) {
			$driver_name_metaquery = array(
				'meta_key' => 'eqs_driver_name',
				'value' => $driver_name
			);
		}
